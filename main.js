...
function playBase(card, p) {
	Readline.question('Base playing');
	playCommon(card, p);
}

function playCard(card, p) {
	playCommon(card, p);
	if (card.hasOwnProperty('base')) { Readline.question('Base running'); }
	if (card.hasOwnProperty('base') || card.hasOwnProperty('outpost')) { process.exit(1); moveCard(card, p.hand, p.bases); }
}
...
function play(p, notp, trade) {
	...
	while(toBuy.length > 0)
	{
		...
		trade.row = trade.row.concat(trade.deck.draw(1));
